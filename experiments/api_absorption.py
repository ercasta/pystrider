...
# `Optional[X]` / `X | None` return annotations -> `returns_optional yes`. Hand-authored here; slice 3
# is the `absorb(module)` tool that generates these from a real library's declared surface.
...
def analyze_with_absorption(src: str, receiver_types: dict[str, str]) -> list[str]:
    ...
    absorbed-optional call result). Returns the source labels of the raising sites."""
    ik = intake_function(src)
    extra = list(API_FACTS) + _resolve_calls(ik, receiver_types)

...
The files under `pystrider/rules/` are the single authored artifact. Nothing in `pystrider` hardcodes a
...
def load(source: str | None = None, *, path: Path | None = None) -> Library:
    """Load the library. Defaults to `pystrider/rules/`; `source` loads text instead, for probes.

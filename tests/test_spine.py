...
The narrative version with its printed evidence is `experiments/pystrider_spine.py`.
...
#: ⚠ Two capabilities LEFT the engine on 2026-08-23, and both had a pin here. The pins
#: stay, STRICT, so the day either comes back they XPASS loudly rather than sitting
#: quietly skipped. docs/transplant.md has the measurement and the upstream citations.
NEEDS_TRAIL = ("`Machine.why` is gone — the scratchpad keeps no history to walk, so "
               "nothing here can answer WHY a proposition holds. Upstream parks the "
               "replacement behind a future memory system.")
NEEDS_GOALS = ("the engine no longer manages goals, so a rule cannot be read backwards "
               "by asserting one. Goal management moved into the corpus "
               "(`../ugm/ugm/rules/bundle.ugm`); a backward reader has to be authored.")
...
def test_the_PERTURBATION_darkens_recognition():
    ...
    f.run()
    (loop,) = f.subjects("for_stmt")
    assert not f.holds("iteration", loop)
...
def test_the_SAME_rule_read_backwards_asks_for_the_STRUCTURE():
    #: ⚠⚠ THIS IS THE WRITE HALF OF THE BET, and it is the most expensive thing the
    #: scratchpad collapse cost us. `m.focus`, `m.GOAL`, `m.SUBGOAL` and `m.chain` are
    #: all gone; there is no line-level repair, because what this test drove was
    #: engine machinery that upstream deliberately moved into the corpus. Kept intact
    #: rather than rewritten, so that whoever authors the backward reader has the
    #: expectation it has to satisfy sitting right here.
    PLUS = "+"

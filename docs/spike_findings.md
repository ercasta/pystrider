...
| **CHOOSE among candidate edits (§3 step 3)** | `choose_repair()` proposes 3 verified edits with a locality×size gradient and uses the public **CHOOSE** firmware mode to pick the most-local/smallest (`guard-base`, fit 1.0), retaining the beaten alternatives (fit 0.7, 0.5) in an auditable trace. |
...
- **The transformation-rule library and backward means-ends.** Step 5 now (a) *materializes
  real edited source* and verifies it by re-intaking the edit (forward "apply + verify by
  re-execution", closed on actual code), and (b) proposes **several** candidate edits, verifies
  each, and uses the public **CHOOSE** firmware mode to pick the graded-best — smallest /
  most-local wins, with an auditable trace of the beaten alternatives (`choose_repair`). What
  remains of design §3 is the **backward half**: the operators are still generated by
  hand-written Python per outcome-type, not **looked up from an effect-keyed operator library by
  backward-`CHAIN`** from the desired outcome. Selection is built; *retrieval* of operators by
  effect is not.

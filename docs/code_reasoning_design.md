...
**Modification materializes real code, and CHOOSE selects among edits.** Step 5 is not just a
semantic effect: `transform.py` rewrites the AST and unparses **actual edited Python**;
`analysis.repair` re-intakes that edited source (guard facts *derived*, not hand-authored) and
re-analyzes — the outcome clears on the real transformed code (intake grew `if VAR is not None:`
recognition to make the round-trip honest). And `analysis.choose_repair` proposes **several**
candidate edits, verifies each by re-execution, and uses the public **CHOOSE** firmware mode to
pick the graded-best (smallest / most-local wins), keeping the beaten alternatives auditable —
design §3 step 3, on the firmware.
...
state-succession probe), the **backward** half of means-ends (operators are generated by
hand-written Python per outcome-type; retrieving them from an *effect-keyed operator library by
backward-`CHAIN`* is unbuilt — selection is built, retrieval is not), the concolic/SMT/type
CALLs, and anything past one function.
...
- **Transformation-rule library** — how are edit operators authored and keyed by effect? Is
  ...
  shared vocabulary is what lets backward-CHAIN connect goal to operator.) *(Spike now proves
  two of the three §3 steps: materialize+verify (step 4) and graded CHOOSE among candidates
  (step 3, via the public CHOOSE mode). Still open: the **effect-keyed operator library +
  backward-CHAIN retrieval** (step 2) — operators are currently hand-generated per outcome-type,
  not looked up by effect.)*

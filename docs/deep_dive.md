...
**end-to-end as probes** (310 tests green). A library's API surface is now **absorbed as data**
...
spec-vs-code divergence derived as a fact and repaired spec-directed. All green (310 tests):
...
the [grammapy convergence](grammapy_convergence.md), where a second in-repo package (`grammapy`)
...
- **Honesty by execution, and a stated contract per verdict.** A fragment's declared write-footprint is
  ...
  does *not* ([`docs/oracle_contracts.md`](oracle_contracts.md)).
